...
impl CodeTheme {
	...
	pub fn from_memory(ctx: &egui::Context) -> Self {
		if ctx.style().visuals.dark_mode {
			ctx.data_mut(|d| d.get_persisted(egui::Id::new("dark")).unwrap_or_else(Self::dark))
		} else {
			ctx.data_mut(|d| d.get_persisted(egui::Id::new("light")).unwrap_or_else(Self::light))
		}
	}
...
impl Default for Highlighter {
	fn default() -> Self {
		...
		ssb
			.add(SyntaxDefinition::load_from_str(include_str!("GLSL.sublime-syntax"), true, None).unwrap());
...
impl Highlighter {
	...
	fn highlight_impl(&self, theme: &CodeTheme, text: &str, language: &str) -> Option<LayoutJob> {
		...
		let syntax =
			self.ps.find_syntax_by_name(language).or_else(|| self.ps.find_syntax_by_extension(language))?;
		...
		for line in LinesWithEndings::from(text) {
			for (style, range) in h.highlight_line(line, &self.ps).ok()? {
				...
				let underline = if underline { egui::Stroke::new(1.0, text_color) } else { egui::Stroke::NONE };
