...
use tetra::graphics::{self, Shader};
use tokio::fs::File;
use turbosql::*;
...
impl egui_tetra2::State<Box<dyn std::error::Error>> for GameState {
		...
		egui::Window::new("Custom Shader").show(egui_ctx, |ui| {
			...
			egui::ScrollArea::vertical().show(ui, |ui| {
				if ui
					...
					.changed()
				{
					self.shader_result = match Shader::from_fragment_string(ctx, &self.shader_string) {
						Ok(shader) => {
							self.shader = shader;
	...
	fn draw(
		...
		graphics::mesh::Mesh::rectangle(
			...
			graphics::Rectangle::new(0.0, 0.0, window_size.0 as f32, window_size.1 as f32),
		)
		.unwrap()
		.draw(ctx, graphics::DrawParams::new());
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	tetra::ContextBuilder::new("ace", 1280, 720)
		.high_dpi(true)
		.show_mouse(true)
		.resizable(true)
		.build()?
		.run(|ctx| Ok(egui_tetra2::StateWrapper::new(GameState::new(ctx)?)))
